...
static TaskHandle_t IrDa_Task_Handle = NULL;/* KEY任务句柄 */
...
static void IrDa_Task(void* pvParameters);/* KEY_Task任务实现 */
...
static void AppTaskCreate(void)
{
  ...
  taskENTER_CRITICAL();           //进入临界区
	...
	xReturn = xTaskCreate((TaskFunction_t )IrDa_Task, /* 任务入口函数 */
                        (const char*    )"IrDa_Task",/* 任务名字 */
                        (uint16_t       )512,   /* 任务栈大小 */
                        (void*          )NULL,	/* 任务入口函数参数 */
                        (UBaseType_t    )2,	    /* 任务的优先级 */
                        (TaskHandle_t*  )&IrDa_Task_Handle);/* 任务控制块指针 */
